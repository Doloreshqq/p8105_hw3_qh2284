...
response is “poor”. Also, we could observe that the “poor” group have
the least variance in both year.
...
combine_longer = combine_data %>%
  pivot_longer(min1:min1440,names_to = "time", values_to = "mins_act") %>%
  na.omit()
```
...
the distribution of education within the male and female.
...
total_ac = combine_data %>%
  mutate(total = rowSums(select(., min1:min1440), na.rm = TRUE)) %>%
  select(seqn, sex, age, bmi, education, total)
...
``` r
# create the plot
plot_24h = ggplot(combine_longer, aes(x = time, y = mins_act, color = sex)) +
  geom_line() +
  facet_wrap(~ education) +
  geom_smooth(se = FALSE) +
